...
const getPosition = course => {
  const position = course.reduce(
    (acc, action) => {
      if (action[0] === 'forward')
        return {
          ...acc,
          horizontal: acc.horizontal + action[1],
          depth: acc.depth + acc.aim * action[1]
        }
      else if (action[0] === 'down') return { ...acc, aim: acc.aim + action[1] }
      else if (action[0] === 'up') return { ...acc, aim: acc.aim - action[1] }
      return { ...acc }
    },
    {
      horizontal: 0,
