...
const externalInput = require('./sonarSweepReport.json');

const puzzleInput = [199, 200, 208, 210, 200, 207, 240, 269, 260, 263];

const getAmountOfSums = (report) => {
  const sum = report.reduce((acc, depth, idx, src) => {
    const currentWindowSum = depth + src[idx + 1] + src[idx + 2];
    if (currentWindowSum > acc.lastSum && idx > 0) {
      acc.sum += 1;
      acc.lastSum = currentWindowSum;
    }
    return { ...acc, lastSum: currentWindowSum };
  }, {
    sum: 0,
    lastSum: 0,
  });
...
console.log(`Amount of sums that are larger than previous sum: ${getAmountOfSums(
  externalInput
  )}`
);