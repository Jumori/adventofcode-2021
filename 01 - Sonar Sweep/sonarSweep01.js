 ...
 * if you can boost its signal strength high enough; there's a little meter
...
const depthMeasurement = report => {
  const measurementIncreases = report.reduce((acc, depth, idx, src) => {
    if (depth > src[idx - 1]) return (acc += 1)
    return acc
  }, 0)
...
console.log(
  `Number of times a depth measurement increases: ${depthMeasurement(
    externalInput
  )}`
)
